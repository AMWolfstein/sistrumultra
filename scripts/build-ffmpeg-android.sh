#!/usr/bin/env bash
# Builds the Android FFmpeg decoder used by androidApp.
...
SDK_DIR="$(awk -F= '/^sdk.dir=/{print $2}' "${REPO_ROOT}/local.properties")"
NDK_VERSION="30.0.15729638"
NDK="${SDK_DIR}/ndk/${NDK_VERSION}"
TOOLCHAIN="$(find "${NDK}/toolchains/llvm/prebuilt" -maxdepth 1 -type d \( -name 'darwin-*' -o -name 'linux-*' -o -name 'windows-*' \) -print -quit)"
